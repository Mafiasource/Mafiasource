...
class GetLanguageContent
{
    ...
    public function registerLangs()
    {
        ...
        $langs = array(
            ...
            'USERNAME_INFO' => "Only letters, numbers or a hyphen character(-), minimal 1 letter. range between 3-15 characters.",
            'INVALID_USERNAME' => "You've entered an invalid username. Only letters, numbers or a hyphen character(-), minimal 1 letter. range between 3-15 characters.",
    ...
    public function recoverPasswordLangs()
    {
        ...
        $langs = array(
            ...
            'RECOVER_PASSWORD_EMAIL_MESSAGE' => "Dear {username}<br /><br />We received a request to recover your lost password, if this wassn't you than you can ignore this email.<br /><br />To set a new password you can click or copy the following URL in your address bar: <a href='".PROTOCOL.strtolower($route->settings['domain'])."/recover-password/key/{key}'>".PROTOCOL.strtolower($route->settings['domainBase'])."/recover-password/key/{key}</a><br />After that follow the instructions on your sreen.<br />If you land on a not found page than the above link is expired. You can make a new request on <a href='".PROTOCOL.strtolower($route->settings['domainBase'])."/recover-password'>".PROTOCOL.strtolower($route->settings['domainBase'])."/recover-password</a><br /><br /><br />With kind regards<br />".ucfirst($route->settings['domainBase']),
    ...
    public function gymLangs()
    {
        $langs = array(
            ...
            'COMPETITION_STAKE_BETWEEN_50_AND_5M' => "You need to stake an amount between $50 and $5.000.000",
    ...
    public function groundLangs()
    {
        $langs = array(
            ...
            'GROUND_OWNER_INFO' => "Buy buildings to earn money every hour! Upgrade your buildings to earn up to 25% more each hour.",
    ...
    public function donationShopLangs()
    {
        ...
        $langs = array(
            ...
            'CONTINUE' => "Continue",
            'CAN_RECEIVE' => "You can receive {credits} more credits.",
            ...
            'DONATE_BTN_FOOT' => "<h4>Trouble?</h4><p>Contact an Administrator for help.</p>",
    ...
    public function forumLangs()
    {
        $langs = array(
            ...
            'REPLY_TOO_SHORT_OR_LONG' => "Your reaction must be between 2 and 3000 characters long.",
            'TOPIC_TOO_SHORT_OR_LONG' => "Your topic must be between 2 and 3000 characters long.",
    ...
    public function familyLangs() // Family list  ,    page, bank  ,    tiny bit family garage (rest in garage langs)   &    management
    {
        ...
        $langs = array(
            ...
            'FAMILY_PROFILE_TO_LONG' => "Your family profile can't be longer than {max} characters (unseen elements excluded, i.e. tables, lists, images..)",
            'FAMILY_MESSAGE_TO_LONG' => "Your family message can't be longer than 10000 characters (unseen elements excluded, i.e. tables, lists, images..)",
    ...
    public function familyRaidLangs()
    {
        ...
        $langs = array(
            ...
            'WAITING' => "Waiting",
            'QUIT' => "Quit",
            'BULLETS_INFO' => "$50 each bullet, between 50 and 1000 bullets!",
    ...
    public function ranksScoreLangs()
    {
        ...
        $langs = array(
            ...
            'RANKS_SCORE_INFO' => "<p>When you're attacked than you can always attack back atleast once within the 24h, no matter what rank.</p><p>* Max. = maximum, Min. = minimum.</p><p>The personal score is being checked and updated every hour.<br />The score is based on your amount of hoes, honor points, kills, ground area's on the ground map and your rank.<br />The more you have the more score you will receive, you can check your score each hour on the <a href='". $route->getRouteByRouteName('status') ."'><strong>status page</strong></a>.</p><p>Scums that haven't been online for over 5 days can always be attacked no matter your rank!</p><p>Dead players that haven't been online for over 7 days will come back to live every night.</p>",
        );
        return $langs;
    }
    ...
    public function settingsLangs()
    {
        ...
        $langs = array(
            ...
            'EMAIL_CHANGE_NEED_TO_VERIFY' => "We've send an email change request to your previous email adress, a tip: {coveredEmail}<br />Beware! The link to change your email will expire in 2 hours from now.",
            'SAME_EMAIL_NO_CHANGE' => "You can't change the email in the current set email.",
            'CHANGE_EMAIL_MESSAGE' => "Dear {username}<br /><br />We received a request to change your email adres into: <strong>{newEmail}</strong>, if you did not request such change you can simply ignore this email.<br />If this is the case it's recommended to change your password asap to avoid further complications.<br /><br />To change your email adres click or copy the following URL in your address bar: <a href='".PROTOCOL.strtolower($route->settings['domain'])."/change-email/key/{key}'>".PROTOCOL.strtolower($route->settings['domainBase'])."/change-email/key/{key}</a><br />After that follow the instructions on your sreen.<br />If you land on a not found page than the above link is expired. You can make a new request in-game on ".strtolower($route->settings['domainBase'])."<br /><br /><br />With kind regards<br />".ucfirst($route->settings['domainBase']),
            ...
            'PROFILE_TO_LONG' => "Your profile can't be longer than {max} characters (unseen elements included, i.e. tables, list elements, images..)",
            ...
            'PRIVATEID_INFO' => "PrivateID allows you to set-up a randomly generated username for your account, once activated you'll only be able to login with your PrivateID in the username field.<br /><strong>Careful!</strong> PrivateID is only visible during each generation after which it's hashed and irreversibly stored in our system. The latest generated PrivateID will always be the correct one unless it got deactivated."
        );
        return $langs;
    }
