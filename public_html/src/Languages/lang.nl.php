...
class GetLanguageContent
{
    ...
    public function recoverPasswordLangs()
    {
        ...
        $langs = array(
            ...
            'RECOVER_PASSWORD_EMAIL_MESSAGE' => "Beste {username}<br /><br />We hebben een aanvraag ontvangen om je verloren wachtwoord te herstellen, als jij dit niet was dan kan je deze email negeren.<br /><br />Om een nieuw wachtwoord in te stellen moet je op volgende URL drukken of deze kopie&euml;ren naar je adres balk: <a href='".PROTOCOL.strtolower($route->settings['domainBase'])."/recover-password/key/{key}'>".PROTOCOL.strtolower($route->settings['domainBase'])."/recover-password/key/{key}</a><br />Daarna volg je de instructies op het scherm.<br /><br />",
            'RECOVER_PASSWORD_EMAIL_MESSAGE_PRIVATEID' => "Een PrivateID is aanwezig op jouw account. Als je uw PrivateID wil deactiveren, dan moet je op volgende URL drukken of deze kopie&euml;ren naar je adres balk: <a href='".PROTOCOL.strtolower($route->settings['domain'])."/recover-password/disable-privateid/{key}'>".PROTOCOL.strtolower($route->settings['domainBase'])."/recover-password/disable-privateid/{key}</a><br /><br />",
            'RECOVER_PASSWORD_EMAIL_FOOTER' => "Indien je op een niet gevonden pagina beland dan zijn bovenstaande link(s) verlopen. Je kan een nieuwe aanvraag doen op <a href='".PROTOCOL.strtolower($route->settings['domainBase'])."'>".PROTOCOL.strtolower($route->settings['domainBase'])."/recover-password</a><br /><br /><br />Met vriendelijke groeten<br />".ucfirst($route->settings['domainBase']),
