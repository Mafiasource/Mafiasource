...
class GetLanguageContent
{
    ...
    public function initBaseLangs()
    {
        ...
        if($user->notIngame())
        {
            $langs = array(
                ...
                'PLAYERS_BEFORE_MSG' => "In totaal gingen <span class='total'>{totalPlayers}</span> criminelen je voor!",
    ...
    public function recoverPasswordLangs()
    {
        ...
        $langs = array(
            ...
            'RECOVER_PASSWORD_EMAIL_MESSAGE' => "Beste {username}<br /><br />We hebben een aanvraag ontvangen om je verloren wachtwoord te herstellen, als jij dit niet was dan kan je deze email negeren.<br /><br />Om een nieuw wachtwoord in te stellen moet je op volgende URL drukken of deze kopie&euml;ren naar je adres balk: <a href='".PROTOCOL.strtolower($route->settings['domainBase'])."/recover-password/key/{key}'>".PROTOCOL.strtolower($route->settings['domainBase'])."/recover-password/key/{key}</a><br />Daarna volg je de instructies op het scherm.<br />Indien je op een niet gevonden pagina beland dan is bovenstaande link verlopen. Je kan een nieuwe aanvraag doen op <a href='".PROTOCOL.strtolower($route->settings['domainBase'])."'>".PROTOCOL.strtolower($route->settings['domainBase'])."/recover-password</a><br /><br /><br />Met vriendelijke groeten<br />".ucfirst($route->settings['domainBase']),
    ...
    public function gymLangs()
    {
        $langs = array(
            ...
            'COMPETITION_STAKE_BETWEEN_50_AND_5M' => "Je moet tussen de $50 en $5.000.000 inzetten.",
    ...
    public function groundLangs()
    {
        $langs = array(
            ...
            'GROUND_OWNER_INFO' => "Koop gebouwen om ieder uur geld te verdienen! Upgrade jouw gebouwen om tot 25% meer te verdienen per uur.",
    ...
    public function forumLangs()
    {
        $langs = array(
            ...
            'REPLY_TOO_SHORT_OR_LONG' => "Je reactie moet tussen de 2 en de 3000 tekens in totaal bevatten.",
            'TOPIC_TOO_SHORT_OR_LONG' => "Je topic bericht moet tussen de 2 en de 3000 tekens in totaal bevatten.",
    ...
    public function familyLangs() // Family list  ,    page, bank  ,    tiny bit family garage (rest in garage langs)   &    management
    {
        ...
        $langs = array(
            ...
            'FAMILY_PROFILE_TO_LONG' => "Je familie profiel mag niet langer zijn dan {max} karakters (verborgen elementen niet-inbegrepen, vb. tebellen, lijsten, afbeeldingen,..)",
            'FAMILY_MESSAGE_TO_LONG' => "Je familie bericht mag niet langer zijn dan 10000 karakters (verborgen elementen niet-inbegrepen, vb. tebellen, lijsten, afbeeldingen,..)",
    ...
    public function familyRaidLangs()
    {
        ...
        $langs = array(
            ...
            'WAITING' => "Wachten",
            'QUIT' => "Stoppen",
            'BULLETS_INFO' => "$50 per kogel, tussen 50 en 1000 kogels!",
    ...
    public function ranksScoreLangs()
    {
        ...
        $langs = array(
            ...
            'RANKS_SCORE_INFO' => "<p>Wordt jij aangevallen, dan kan je altijd minstens eenmalig terug aanvallen binnen de 24u, ongeacht je rank.</p><p>* Max. = maximaal, Min. = minimaal.</p><p>De persoonlijke score wordt elk uur gecontroleerd en bijgewerkt.<br />De score is gebaseerd op jouw aantal hoeren, eerpunten, kills, landjes op de plattegrond en je rank.<br />Hoe meer je er hebt hoe meer score je ontvangt, je kan jouw score per uur controlleren op de <a href='". $route->getRouteByRouteName('status') ."'><strong>status pagina</strong></a>.</p><p>Scums die langer dan 5 dagen niet online gekomen zijn kan je altijd aanvallen ongeacht je rank!</p><p>Dode spelers die langer dan 7 dagen niet online zijn gekomen worden iedere nacht terug tot leven gewekt.</p>",
        );
        return $langs;
    }
    ...
    public function settingsLangs()
    {
        ...
        $langs = array(
            ...
            'EMAIL_CHANGE_NEED_TO_VERIFY' => "We hebben je een email verstuurd met verdere instructies naar je oude email adres, kleine tip: {coveredEmail}<br />Pas op! De link om uw email aan te passen zal vervallen binnen 2 uren vanaf nu.",
            'SAME_EMAIL_NO_CHANGE' => "Je kan het email adres niet veranderen omdat dit het huidige email adres is.",
            'CHANGE_EMAIL_MESSAGE' => "Beste {username}<br /><br />We hebben een aanvraag ontvangen om je email adres te veranderen in: <strong>{newEmail}</strong>, indien je zelf deze aanvraag niet hebt gedaan kan je deze email simpel weg negeren.<br />Indien dit het geval is raden we je aan om je wachtwoord zo snel mogelijk te veranderen om verdere problemen te voorkomen.<br /><br />Om je email adres aan te passen moet je op volgende URL drukken of deze kopie&euml;ren naar je adres balk: <a href='".PROTOCOL.strtolower($route->settings['domainBase'])."/change-email/key/{key}'>".PROTOCOL.strtolower($route->settings['domainBase'])."/change-email/key/{key}</a><br />Daarna volg je de instructies op het scherm.<br />Indien je op een niet gevonden pagina beland dan is bovenstaande link verlopen. Je kan een nieuwe aanvraag doen in-game op ".strtolower($route->settings['domainBase'])."<br /><br /><br />Met vriendelijke groeten<br />".ucfirst($route->settings['domainBase']),
            ...
            'PROFILE_TO_LONG' => "Je profiel mag niet langer zijn dan {max} karakters (verborgen elementen inbegrepen, vb. tebellen, lijst opsomming, afbeeldingen,..)",
            ...
            'PASSWORD_CHANGE_SUCCESS' => "Je wachtwoord werd succesvol aangepast.",
            ...
            'PRIVATEID_INFO' => "Met PrivateID kunt u een willekeurig gegenereerde gebruikersnaam voor uw account instellen, eenmaal geactiveerd kunt u alleen inloggen met uw PrivateID in het gebruikersnaamveld.<br /><strong>Voorzichtig!</strong> PrivateID is alleen zichtbaar tijdens elke generatie waarna het gehashed en onomkeerbaar wordt opgeslagen in ons systeem. De laatst gegenereerde PrivateID is altijd de juiste, tenzij deze werd gedeactiveerd.",
        );
        return $langs;
    }
