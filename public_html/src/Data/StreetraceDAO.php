...
class StreetraceDAO extends DBConfig
{
    ...
    public function createRace($type, $stake, $requiredPlayers, array $vehicle, $stateId)
    {
        ...
        try
        {
            ...
            $statement = $this->dbh->prepare(
                "INSERT INTO `streetrace_game` (`organizerID`, `type`, `stake`, `requiredPlayers`, `stateID`, `status`, `created`) VALUES (:uid, :type, :stake, :requiredPlayers, :stateID, 'open', NOW())"
            );
    ...
    {
        $row = $this->con->getDataSR(
            "SELECT g.`id`, g.`type`, g.`stake`, g.`requiredPlayers`, g.`status`, g.`created`, g.`started`, g.`finished`, g.`organizerID`, g.`stateID`, s.`name` AS `stateName`, u.`username` AS `organizerName` FROM `streetrace_game` AS g LEFT JOIN `user` AS u ON (g.`organizerID` = u.`id`) LEFT JOIN `state` AS s ON (g.`stateID` = s.`id`) WHERE g.`id` = :raceID",
            array(':raceID' => $raceId)
        );
