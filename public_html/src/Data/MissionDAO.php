...
use src\Entities\Mission;

class MissionDAO extends DBConfig
{
    protected $con = ""; // Init
    ...
    public function getProfileMissionsByUserID($uid)
    {
        if(isset($_SESSION['UID']))
        {
            $missionService = new MissionService();
            $missionTiers = $missionService->missionTiers;
            $tiersProgress = $this->getMissionTiersAndProgress($uid);
            $list = array();
